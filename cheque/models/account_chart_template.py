...
class AccountChartTemplate(models.AbstractModel):
    ...
    def _get_cheque_account_journal(self, template_code):
            ...
            return {
                "third_party_check": {
                    ...
                    'outbound_payment_method_line_ids': [
                        Command.create({
                            'payment_method_id': self.env.ref('cheque.account_payment_method_out_third_party_checks').id,
                            'payment_account_id': 'base_outstanding_payments',
                        }),
                    ],
                    'inbound_payment_method_line_ids': [
                        Command.create({
                            'payment_method_id': self.env.ref('cheque.account_payment_method_new_third_party_checks').id,
                            'payment_account_id': 'base_outstanding_receipts',
                        }),
                        Command.create({
                            'payment_method_id': self.env.ref('cheque.account_payment_method_in_third_party_checks').id,
                            'payment_account_id': 'base_outstanding_receipts',
                        }),
                    ],
                },
    ...
    def _get_cheque_outstanding_account_account(self, template_code):
        if self.env.company.country_id.code in self._get_third_party_checks_country_codes():
            return {

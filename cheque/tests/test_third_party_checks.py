...
class TestThirdChecks(L10nLatamCheckTest):
    ...
    def create_third_party_check(self, journal=False, check_numbers=['00000001', '00000002']):
        ...
        vals = {
            ...
            'payment_method_line_id': journal._get_available_payment_method_lines('inbound').filtered(lambda x: x.code == 'cheque_incoming').id,
        }
    ...
    def test_02_third_party_check_delivery(self):
        ...
        vals = {
            ...
            'payment_method_line_id': self.third_party_check_journal._get_available_payment_method_lines('outbound').filtered(lambda x: x.code in ('cheque_existing_out', 'cheque_return')).id,
        }
        ...
        vals = {
            ...
            'payment_method_line_id': self.rejected_check_journal._get_available_payment_method_lines('inbound').filtered(lambda x: x.code == 'cheque_existing_in').id,
        }
        ...
        vals = {
            ...
            'payment_type': 'outbound',
            'journal_id': self.rejected_check_journal.id,
            'payment_method_line_id': self.rejected_check_journal._get_available_payment_method_lines('outbound').filtered(lambda x: x.code in ('cheque_existing_out', 'cheque_return')).id,
        }
    ...
    def test_03_deposit(self):
        ...
        self.env['account.payment'].create({
            ...
            'journal_id': self.rejected_check_journal.id,
            'l10n_latam_move_check_ids': [Command.set([check.id])],
            'payment_method_line_id': self.rejected_check_journal._get_available_payment_method_lines('inbound').filtered(lambda x: x.code == 'cheque_incoming').id,
        }).action_post()
    ...
    def test_05_check_current_journal_with_both_operations(self):
        ...
        outbound_payment = self.env['account.payment'].create({
            ...
            'payment_method_line_id': self.third_party_check_journal
                ...
                .filtered(lambda x: x.code == 'cheque_existing_out').id,
        })

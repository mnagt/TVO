...
class PaymentMassTransfer(models.TransientModel):
    _name = 'payment.mass.transfer'
    ...
    check_ids = fields.Many2many(
        'account.cheque', 'tranfer_check_rel'
    ...
    def default_get(self, fields_list):
        ...
        if 'check_ids' in fields_list and 'check_ids' not in res:
            if self._context.get('active_model') != 'account.cheque':
                raise UserError(_("The register payment wizard should only be called on account.payment records."))
            checks = self.env['account.cheque'].browse(self._context.get('active_ids', []))
            if checks.filtered(lambda x: x.payment_method_line_id.code != 'cheque_incoming'):
    ...
    def _create_payments(self):
        ...
        self.ensure_one()
        checks = self.check_ids.filtered(lambda x: x.payment_method_line_id.code == 'cheque_incoming' and x.currency_id == self.check_ids[0].currency_id)
        ...
        pay_method_line = self.journal_id._get_available_payment_method_lines('outbound').filtered(
            lambda x: x.code in ('cheque_existing_out', 'cheque_return')
        )[:1]
        ...
        outbound_payment = self.env['account.payment'].create({
            ...
            'move_cheque_ids': [Command.link(x.id) for x in checks],
        })
        ...
        dest_payment_method = self.destination_journal_id.inbound_payment_method_line_ids.filtered(
            lambda x: x.code == 'cheque_existing_in'
        )

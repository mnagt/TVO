...
class PDC_wizard(models.Model):
    ...
    def open_journal_entry(self):
        [action] = self.env.ref(
            'jm_pdc.sh_pdc_action_move_journal_line').read()
    ...
    payment_date = fields.Date(
        "Date", default=fields.Date.today(), required=1,tracking=True)
...
#         bank_account = self.journal_id.payment_debit_account_id.id or self.journal_id.payment_credit_account_id.id
    ...
    def notify_customer_due_date(self):
        ...
        if self.env.company.is_cust_due_notify:
            ...
            view = self.env.ref("jm_pdc.sh_pdc_payment_form_view",raise_if_not_found = False).sudo()
            ...
            for record in records:
                if (record.due_date == notify_date_1
                    ...
                    if self.env.company.is_notify_to_customer:
                        ...
                        #     'jm_pdc', 'sh_pdc_company_to_customer_notification_1'
                        #     )
                        template_download_id = self.env.ref('jm_pdc.sh_pdc_company_to_customer_notification_1')
                    ...
                    if self.env.company.is_notify_to_user and self.env.company.sh_user_ids:
                        ...
                        # template_download_id = record.env['ir.model.data'].get_object(
                        #     'jm_pdc', 'sh_pdc_company_to_int_user_notification_1'
                        #     )
                        template_download_id = self.env.ref('jm_pdc.sh_pdc_company_to_int_user_notification_1')
